		...
		for ( int j = 0; j < GRIDS_X; j++ )
		{
			for ( int i = 0; i < GRIDS_X; i++ )
			{
				if ( j < 2 and i < halfx + 3 and i >= halfx - 3 and 
					...
					host_obstacle[0][IX(i,j,k,GRIDS_X,GRIDS_X,GRIDS_X)] = MACRO_BOUNDARY_SOURCE;
			}
		}
	}

	cudaMemcpy( dev_obstacle[0], host_obstacle[0], sizeof(double)*GRIDS_X*GRIDS_X*GRIDS_X, cudaMemcpyHostToDevice );

	if ( helper.GetCUDALastError( "call member function InitBoundary failed", __FILE__, __LINE__ ) )
	{
		FreeResource();
		exit(1);
	}
...
//	{
//		kernelAddSource __device_func__ ( dev_den, dev_u, dev_v, dev_w, dev_obstacle[0], DENSITY, VELOCITY, DELTATIME, time(NULL) );
//
//		if ( helper.GetCUDALastError( "device kernel: kernelPickData failed", __FILE__, __LINE__ ) )
//		{
//			FreeResource();
...
{
	if ( add ) m_scSolver.SolveSource( dev_den, dev_u, dev_v, dev_w, dev_obstacle[0], 
		&increase_times, &decrease_times, timestep );

//	m_scSolver.SolveVelocity( dev_u0, dev_v0, dev_w0, dev_u, dev_v, dev_w, dev_div, dev_p, timestep );
//	m_scSolver.SolveDensity( dev_den0, dev_den, dev_u, dev_v, dev_w, timestep );
...
{
	m_scHelper.DeviceDim3D( &blockDim, &gridDim, THREADS_X, TILE_X, GRIDS_X, GRIDS_X, GRIDS_X );

	kernelAddSource __device_func__ ( ptrDevDens, ptrDevU, ptrDevV, ptrDevW, ptrDevObst, DENSITY, VELOCITY, DELTATIME, time(NULL) );
};
			...
			cdouble *ptrDevU, cdouble *ptrDevV, cdouble *ptrDevW, cdouble timestep )
{
	...
	Advection ( ptrDevDens, ptrDevDens0, timestep, ptrDevU, ptrDevV, ptrDevW );
};
