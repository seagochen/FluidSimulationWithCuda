...
__global__ void kernelPickData ( unsigned char *data, float const *grid )
{
	GetIndex();
...
__global__ void kernelPickData ( unsigned char *data, float const *grid1, float const *grid2, float const *grid3 )
{
	GetIndex();
...
__global__ void kernelAddSource ( float *dens, float *u, float *v, float*w )
{
	...
	if (i < 5 ) if ( j > half - 10 && j < half + 10 ) if ( k > half - 10 && k < half + 10 )
	{
		// Red
		if ( u != NULL ) u [ Index(i,j,k) ] = 1.f;
	}
	if (j < 5) if ( i > half - 10 && i < half + 10 ) if ( k > half - 10 && k < half + 10 )
	{
		// Green
//		if ( v != NULL ) v [ Index(i,j,k) ] = 25.f;
	}
	if (k < 5) if ( i > half - 10 && i < half + 10 ) if ( j > half - 10 && j < half + 10 )
	{
		// Blue
//		if ( w != NULL ) w [ Index(i,j,k) ] = 35.f;
	}
};

__global__ void kernelAdvection ( float *grid, float const *grid_in, float const *u_in, float const *v_in, float const *w_in, float const delta_time )
{
	GetIndex();
	
	float u = i + u_in [ Index(i,j,k) ] * delta_time;
	float v = j + v_in [ Index(i,j,k) ] * delta_time;
	float w = k + w_in [ Index(i,j,k) ] * delta_time;
	float temp = grid_in [ Index(i,j,k) ];

	int gx = myRound (u);
	int gy = myRound (v);
	int gz = myRound (w);
	
	if ( gx < Grids_X && gx >= 0 ) if ( gy < Grids_X && gy >= 0 ) if ( gz < Grids_X && gz >= 0 )
	{
		float rate = myInterpolateRate (u, v, w, gx, gy, gz, 1.f);
		if ( rate >= 0.f && rate <= 1.f )
			grid [ Index(gx, gy, gz) ] = rate * temp;
		else
			grid [ Index(gx, gy, gz) ] = 0.f;
	}
};
...
//	kernelAddSource cudaDevice(gridDim, blockDim) ( NULL, dev_u, NULL, NULL );
	kernelAdvection cudaDevice(gridDim, blockDim) ( dev_grid, dev_u, dev_u, dev_v, dev_w, 4 );
//	kernelCopyBuffer cudaDevice(gridDim, blockDim) ( dev_u, dev_grid );
//	kernelZeroBuffer cudaDevice(gridDim, blockDim) ( dev_grid );
   
	//kernelPickData cudaDevice(gridDim, blockDim) ( dev_data, dev_u, dev_v, dev_w );
	kernelPickData cudaDevice(gridDim, blockDim) ( dev_data, dev_grid );
...
inline __host__ __device__ int myRound ( float const value )
{
	float flo = floor(value);
	float rem = value - flo;
	if ( rem >= 0.5f ) return (int)flo + 1;
	else return (int)flo;
};

__device__ float myLookupCell ( float const *grid, float const u, float const v, float const w )
{
	int i = myRound ( u );
	int j = myRound ( v );
	int k = myRound ( w );

	if ( i >= Grids_X ) return 0.f; if ( i < 0.f ) return 0.f;
	if ( j >= Grids_X ) return 0.f; if ( j < 0.f ) return 0.f;
	if ( k >= Grids_X ) return 0.f; if ( k < 0.f ) return 0.f;

	return grid [ Index(i,j,k) ];
};


__device__ float myInterpolateRate ( float const bx, float const by, float const bz, int const gx, int const gy, int const gz, float const radius )
{
	if ( radius < 0.f ) return 0.f;

	float const rate = 1 / radius;
